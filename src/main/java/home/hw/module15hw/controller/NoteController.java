package home.hw.module15hw.controller;

import home.hw.module15hw.model.Note;
import home.hw.module15hw.sevice.NoteService;
...
public class NoteController {
    ...
    public void noteEdit(@RequestParam(required = false, defaultValue = "0") long id
        ...
        response.getWriter().write("<html lang='uk'><head><meta charset='UTF-8'><title>Edit note</title>" +
                ...
                "<input type='hidden' name='id' value='" + (note != null ? note.getId() : "") + "'>" +
                "<div class='form-group'><label for='title'>Title</label>" +
                "<input type='text' id='title' name='title' class='form-control' value='" + (note != null ? note.getTitle() : "") + "'></div>" +
                "<div class='form-group'><label for='content'>Content:</label>" +
                "<textarea id='content' name='content' class='form-control'>" + (note != null ? note.getContent() : "") + "</textarea></div>" +
